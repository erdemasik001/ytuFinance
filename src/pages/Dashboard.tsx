// Remove React import as it is unused
import {
    TrendingUp,
    TrendingDown,
    Wallet,
...
const StatCard = ({ title, value, trend, trendUp, icon: Icon, colorClass }: any) => (
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-lg shadow-blue-900/5 dark:shadow-blue-900/10 border border-gray-100 dark:border-slate-700/50 card-hover group relative overflow-hidden">
        ...
        <div className="flex justify-between items-start mb-4 relative z-10">
            <div className={`p-3.5 rounded-2xl ${colorClass} bg-opacity-20 text-opacity-100`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-lg shadow-blue-900/5 dark:shadow-blue-900/10 border border-gray-100 dark:border-slate-700/50 p-8 card-hover">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Yaklaşan Ödemeler</h2>
                            <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Bu ay yapılacak ödemeleriniz</p>
                        </div>
                        <button className="px-4 py-2 bg-gray-50 hover:bg-gray-100 dark:bg-slate-700 dark:hover:bg-slate-600 text-gray-600 dark:text-slate-300 text-sm font-semibold rounded-xl transition-colors">Tümünü Gör</button>
                    </div>
                    <div className="space-y-4">
                        {[1, 2, 3].map((_, i) => (
                            <div key={i} className="group flex items-center justify-between p-4 rounded-2xl hover:bg-slate-700/50 transition-colors border border-transparent hover:border-slate-700">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-gray-900 dark:text-white text-base">Harput Yemek</h4>
                                        <p className="text-sm text-gray-500 dark:text-slate-400 font-medium">15 Şubat 2026</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <span className="block font-bold text-gray-900 dark:text-white text-lg">₺4,200</span>
                        ...
                        {[1, 2, 3].map((_, i) => (
                            <div key={i} className="group flex items-center justify-between p-4 rounded-2xl hover:bg-slate-700/50 transition-colors border border-transparent hover:border-slate-700">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-gray-900 dark:text-white text-base">Umut Arslan</h4>
                                        <p className="text-sm text-gray-500 dark:text-slate-400 font-medium">EKB Çizim Bedeli</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <span className="block font-bold text-green-600 dark:text-green-400 text-lg">+₺15,000</span>
                                    <span className="text-xs font-semibold text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-500/10 px-2 py-0.5 rounded-md border border-transparent dark:border-green-500/20">Tahsil Edildi</span>
                                </div>
                            </div>
                        ))}
                    </div>
